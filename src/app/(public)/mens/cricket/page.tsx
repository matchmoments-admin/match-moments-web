...
export const metadata = {
  title: "Men's Cricket | Match Moments",
  description: "Complete coverage of men's cricket - ICC World Cup, IPL & more",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Cricket&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Cricket&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Cricket&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function MensCricketPage() {
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: 'Cricket', href: '/mens/cricket', current: true },
          ]}
        />
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-600 to-blue-800 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                ICC World Cup, IPL, and men's cricket worldwide
              </p>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Men's Cricket Competitions"
              description="ICC tournaments, IPL & more"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {competitions.slice(0, 6).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/mens/cricket/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Latest cricket matches"
            />
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Men's Cricket Moments"
              description="Best sixes and wickets"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No men's cricket data available at the moment.</p>
