...
export const metadata = {
  title: "Men's Soccer | Match Moments",
  description: "Complete coverage of men's soccer competitions worldwide",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Soccer&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Soccer&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Soccer&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function MensSoccerPage() {
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: 'Soccer', href: '/mens/soccer', current: true },
          ]}
        />
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-green-600 to-green-800 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                Premier League, Champions League, La Liga, Serie A & more
              </p>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Elite Men's Soccer Competitions"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {competitions.slice(0, 6).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/mens/soccer/competitions/${competition.Id}`}
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {matches.map((match: any) => (
                <Link
                  key={match.Id}
                  href={`/mens/soccer/fixtures/${match.Id}`}
                  className="rounded-3xl bg-white border border-gray-200 p-6 hover:border-black transition-colors"
                >
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Men's Soccer Moments"
              description="The best goals, saves, and plays from men's soccer"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No men's soccer data available at the moment.</p>
