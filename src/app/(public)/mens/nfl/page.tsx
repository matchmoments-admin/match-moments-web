...
export const metadata = {
  title: "Men's American Football | Match Moments",
  description: "Complete coverage of men's American football - NFL & more",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=American Football&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=American Football&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=American Football&gender=Men's Team&limit=10`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function MensNFLPage() {
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: 'American Football', href: '/mens/nfl', current: true },
          ]}
        />
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-700 to-purple-900 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                NFL, college football & men's American football worldwide
              </p>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Men's Football Competitions"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {competitions.slice(0, 6).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/mens/nfl/competitions/${competition.Id}`}
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Men's Football Moments"
              description="Best touchdowns and plays"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No men's American football data available at the moment.</p>
