...
export const metadata = {
  title: "Women's Tennis | Match Moments",
  description: "Complete coverage of women's tennis - WTA Tour, Grand Slams & more",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Tennis&gender=Women's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Tennis&gender=Women's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Tennis&gender=Women's Team&limit=10`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function WomensTennisPage() {
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: 'Tennis', href: '/womens/tennis', current: true },
          ]}
        />
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-yellow-500 to-yellow-700 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                WTA Tour, Grand Slams & women's tennis worldwide
              </p>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Elite Women's Tennis Tournaments"
              viewAllHref="/womens/tennis/competitions"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {competitions.slice(0, 6).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/womens/tennis/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              viewAllHref="/womens/tennis/fixtures"
              viewAllText="View All"
            />
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {matches.map((match: any) => (
                <Link
                  key={match.Id}
                  href={`/womens/tennis/fixtures/${match.Id}`}
                  className="rounded-3xl bg-white border border-gray-200 p-6 hover:border-black transition-colors"
                >
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Women's Tennis Moments"
              description="The best aces, winners, and rallies from women's tennis"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No women's tennis data available at the moment.</p>
