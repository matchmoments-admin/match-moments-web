...
export const metadata = {
  title: "Women's Cricket | Match Moments",
  description: "Complete coverage of women's cricket - ICC Women's World Cup & more",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Cricket&gender=Women's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Cricket&gender=Women's Team&limit=10`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Cricket&gender=Women's Team&limit=10`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function WomensCricketPage() {
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: 'Cricket', href: '/womens/cricket', current: true },
          ]}
        />
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-600 to-blue-800 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                ICC Women's World Cup and women's cricket worldwide
              </p>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Elite Women's Cricket Competitions"
              viewAllHref="/womens/cricket/competitions"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {competitions.slice(0, 6).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/womens/cricket/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              viewAllHref="/womens/cricket/fixtures"
              viewAllText="View All"
            />
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {matches.map((match: any) => (
                <Link
                  key={match.Id}
                  href={`/womens/cricket/fixtures/${match.Id}`}
                  className="rounded-3xl bg-white border border-gray-200 p-6 hover:border-black transition-colors"
                >
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Women's Cricket Moments"
              description="The best sixes, wickets, and plays from women's cricket"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No women's cricket data available at the moment.</p>
