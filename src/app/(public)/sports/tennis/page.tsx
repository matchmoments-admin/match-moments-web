...
export const metadata = {
  title: "Tennis | Match Moments",
  description: "Complete coverage of tennis tournaments worldwide - Grand Slams, ATP, WTA & more",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Tennis&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Tennis&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Tennis&limit=12`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function TennisPage() {
  ...
  const sport = SPORT_METADATA.tennis;
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: sport.name, href: '/sports/tennis', current: true },
          ]}
        />

        <section className="mb-12">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-yellow-500 to-yellow-700 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                Grand Slam tournaments, ATP, WTA, and tennis competitions worldwide
              </p>
              <div className="flex gap-8">
                ...
                <div>
                  ...
                  <div className="text-sm opacity-75">Tournaments</div>
                </div>
                <div>
        ...
        <section className="mb-12">
          <div className="grid md:grid-cols-2 gap-6">
            <Link href="/womens/tennis" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-black p-8 text-white transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Women's Tennis</h3>
                <p className="text-lg opacity-90">WTA Tour & Grand Slams</p>
              </div>
            </Link>
            <Link href="/mens/tennis" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-gray-100 p-8 text-black transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Men's Tennis</h3>
                <p className="text-lg opacity-75">ATP Tour & Grand Slams</p>
              </div>
            </Link>
          </div>
        </section>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Grand Slams and elite tennis tournaments"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {competitions.slice(0, 8).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/sports/tennis/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Latest tennis matches and upcoming fixtures"
            />
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Tennis Moments"
              description="The best aces, winners, and rallies from tennis"
            />
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No tennis data available at the moment.</p>
            <p className="text-gray-400 mt-2">Check back soon for the latest tournaments and matches!</p>
          </div>
        )}
      </div>
    </main>
