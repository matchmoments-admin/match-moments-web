...
export const metadata = {
  title: "Pickleball | Match Moments",
  description: "Complete coverage of pickleball - the fastest growing sport in America",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Pickleball&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Pickleball&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Pickleball&limit=12`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function PickleballPage() {
  ...
  const sport = SPORT_METADATA.pickleball;
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: sport.name, href: '/sports/pickleball', current: true },
          ]}
        />
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-pink-600 to-pink-800 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                America's fastest growing sport - fun, social, and competitive
              </p>
        ...
        <section className="mb-12">
          <div className="grid md:grid-cols-2 gap-6">
            <Link href="/womens/pickleball" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-black p-8 text-white transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Women's Pickleball</h3>
                <p className="text-lg opacity-90">Professional women's pickleball</p>
              </div>
            </Link>
            <Link href="/mens/pickleball" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-gray-100 p-8 text-black transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Men's Pickleball</h3>
                <p className="text-lg opacity-75">Professional men's pickleball</p>
              </div>
            </Link>
          </div>
        </section>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Top pickleball tournaments"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {competitions.slice(0, 8).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/sports/pickleball/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Latest pickleball matches"
            />
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Pickleball Moments"
              description="Best shots and rallies"
            />
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-16 bg-white rounded-3xl border border-gray-200">
            ...
            <p className="text-gray-600 max-w-md mx-auto">
              Pickleball coverage is coming soon to Match Moments. The fastest growing sport in America!
            </p>
          </div>
        )}
      </div>
