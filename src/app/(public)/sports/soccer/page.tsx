...
export const metadata = {
  title: "Soccer | Match Moments",
  description: "Complete coverage of soccer competitions worldwide - both men's and women's soccer",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Soccer&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Soccer&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Soccer&limit=12`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function SoccerPage() {
  ...
  const sport = SPORT_METADATA.soccer;
      ...
      <div className="container-main py-8">
        ...
        <BreadcrumbNav
          items={[
            ...
            { label: sport.name, href: '/sports/soccer', current: true },
          ]}
        />
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-green-600 to-green-800 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                The world's most popular sport - complete coverage of men's and women's soccer
              </p>
        ...
        <section className="mb-12">
          <div className="grid md:grid-cols-2 gap-6">
            <Link href="/womens/soccer" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-black p-8 text-white transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Women's Soccer</h3>
                <p className="text-lg opacity-90">World Cup, WSL, NWSL & more</p>
              </div>
            </Link>
            <Link href="/mens/soccer" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-gray-100 p-8 text-black transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Men's Soccer</h3>
                <p className="text-lg opacity-75">Premier League, Champions League & more</p>
              </div>
            </Link>
          </div>
        </section>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Elite Soccer Competitions"
              description="The world's top soccer leagues and tournaments"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {competitions.slice(0, 8).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/sports/soccer/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Latest soccer matches and upcoming fixtures"
            />
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Soccer Moments"
              description="The best goals, saves, and plays from soccer"
            />
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No soccer data available at the moment.</p>
