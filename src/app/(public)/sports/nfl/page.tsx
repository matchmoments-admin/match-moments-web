...
export const metadata = {
  title: "American Football | Match Moments",
  description: "Complete coverage of American Football - NFL and more",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=American Football&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=American Football&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=American Football&limit=12`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function NFLPage() {
  ...
  const sport = SPORT_METADATA.nfl;
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: sport.name, href: '/sports/nfl', current: true },
          ]}
        />
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-700 to-purple-900 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                NFL, college football, and American football competitions worldwide
              </p>
        ...
        <section className="mb-12">
          <div className="grid md:grid-cols-2 gap-6">
            <Link href="/womens/nfl" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-black p-8 text-white transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Women's Football</h3>
                <p className="text-lg opacity-90">Women's leagues & flag football</p>
              </div>
            </Link>
            <Link href="/mens/nfl" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-gray-100 p-8 text-black transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Men's Football</h3>
                <p className="text-lg opacity-75">NFL & college football</p>
              </div>
            </Link>
          </div>
        </section>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="American Football Competitions"
              description="Top football leagues and tournaments"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {competitions.slice(0, 8).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/sports/nfl/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Latest American football games and upcoming matchups"
            />
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Football Moments"
              description="The best touchdowns, tackles, and plays from football"
            />
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No American football data available at the moment.</p>
