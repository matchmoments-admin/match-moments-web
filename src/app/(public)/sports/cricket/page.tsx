...
export const metadata = {
  title: "Cricket | Match Moments",
  description: "Complete coverage of cricket competitions worldwide - ICC, IPL & more",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Cricket&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Cricket&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Cricket&limit=12`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function CricketPage() {
  ...
  const sport = SPORT_METADATA.cricket;
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: sport.name, href: '/sports/cricket', current: true },
          ]}
        />

        <section className="mb-12">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-600 to-blue-800 p-12 text-white">
            <div className="relative z-10">
              ...
              <p className="text-xl opacity-90 mb-6">
                The gentleman's game - ICC World Cup, IPL, and cricket leagues worldwide
              </p>
        ...
        <section className="mb-12">
          <div className="grid md:grid-cols-2 gap-6">
            <Link href="/womens/cricket" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-black p-8 text-white transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Women's Cricket</h3>
                <p className="text-lg opacity-90">ICC Women's World Cup & more</p>
              </div>
            </Link>
            <Link href="/mens/cricket" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-gray-100 p-8 text-black transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Men's Cricket</h3>
                <p className="text-lg opacity-75">ICC World Cup, IPL & more</p>
              </div>
            </Link>
          </div>
        </section>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Top cricket leagues and tournaments worldwide"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {competitions.slice(0, 8).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/sports/cricket/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Latest cricket matches and upcoming fixtures"
            />
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Cricket Moments"
              description="The best sixes, wickets, and plays from cricket"
            />
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No cricket data available at the moment.</p>
