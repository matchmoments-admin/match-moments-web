...
export const metadata = {
  title: "Padel | Match Moments",
  description: "Complete coverage of padel competitions worldwide - the fastest growing racquet sport",
};
...
async function getPageData() {
  try {
    ...
    const [competitionsRes, matchesRes, momentsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sports/competitions?sport=Padel&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/matches?sport=Padel&limit=12`, {
        cache: 'no-store',
      }),
      fetch(`${baseUrl}/api/sports/moments/trending?sport=Padel&limit=12`, {
        cache: 'no-store',
      }),
    ]);
...
export default async function PadelPage() {
  ...
  const sport = SPORT_METADATA.padel;
      ...
      <div className="container-main py-8">
        <BreadcrumbNav
          items={[
            ...
            { label: sport.name, href: '/sports/padel', current: true },
          ]}
        />

        <section className="mb-12">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-teal-600 to-teal-800 p-12 text-white">
            <div className="relative z-10">
              <div className="mb-4">
                <span className="inline-block rounded bg-white/20 px-3 py-1 text-xs font-bold uppercase tracking-wide">
                  {sport.status === 'coming-soon' ? 'Coming Soon' : 'Growing Fast'}
                </span>
              </div>
              ...
              <p className="text-xl opacity-90 mb-6">
                The fastest growing racquet sport - combining tennis and squash
              </p>
        ...
        <section className="mb-12">
          <div className="grid md:grid-cols-2 gap-6">
            <Link href="/womens/padel" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-black p-8 text-white transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Women's Padel</h3>
                <p className="text-lg opacity-90">Professional women's padel tours</p>
              </div>
            </Link>
            <Link href="/mens/padel" className="group">
              <div className="relative overflow-hidden rounded-3xl bg-gray-100 p-8 text-black transition-transform hover:scale-[1.02]">
                <h3 className="text-3xl font-bold mb-2">Men's Padel</h3>
                <p className="text-lg opacity-75">Professional men's padel tours</p>
              </div>
            </Link>
          </div>
        </section>
        ...
        {competitions.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Padel Competitions"
              description="Top padel tournaments worldwide"
            />
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {competitions.slice(0, 8).map((competition: any) => (
                <Link
                  key={competition.Id}
                  href={`/sports/padel/competitions/${competition.Id}`}
        ...
        {matches.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              ...
              description="Latest padel matches and upcoming fixtures"
            />
        ...
        {moments.length > 0 && (
          <section className="mb-12">
            <SectionHeader
              title="Trending Padel Moments"
              description="The best shots and plays from padel"
            />
        ...
        {competitions.length === 0 && matches.length === 0 && moments.length === 0 && (
          <div className="text-center py-16 bg-white rounded-3xl border border-gray-200">
            ...
            <p className="text-gray-600 max-w-md mx-auto">
              Padel coverage is coming soon to Match Moments. This fast-growing sport is taking the world by storm!
            </p>
          </div>
        )}
      </div>
