...
export async function getSalesPipeline() {
  ...
  const pipelineData = stages.map((stage) => {
    const stageData = result.records.find((r: any) => r.StageName === stage) || {
      count: 0,
      total: 0,
    };
...
export async function getForecast(months: number = 6) {
  ...
  for (let i = 0; i < months; i++) {
    ...
    const record = result.records[0] || { count: 0, total: 0, weighted: 0 };
...
export async function getSalesMetrics() {
  ...
  const pipelineResult = await conn.query(`
    ...
    WHERE IsClosed = false
  `);
  const totalPipeline = pipelineResult.records[0]?.total || 0;

  // Win rate (last 90 days)
  const ninetyDaysAgo = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  const winRateResult = await conn.query(`
    SELECT 
      COUNT(Id) total,
  ...
  // Average deal size
  const avgDealResult = await conn.query(`
    SELECT AVG(Amount) avg
    FROM Opportunity
    WHERE IsWon = true
      AND CloseDate >= ${ninetyDaysAgo}
  `);
  const avgDealSize = avgDealResult.records[0]?.avg || 0;
  ...
  const cycleResult = await conn.query(`
    ...
    WHERE IsWon = true
      AND CloseDate >= ${ninetyDaysAgo}
  `);
  const avgSalesCycle = cycleResult.records[0]?.avg || 0;
