...
/**
 * Revenue metrics with comprehensive financial analysis
 */
export async function getRevenueMetrics(params: {
  ...
  return getCached(
    ...
    async () => {
      ...
      const revenueData = await client.query<any>(`
        SELECT 
      ...
      `);
      const activeCustomers = customerResult[0]?.count || 0;

      // New customers this month
      const newCustomersResult = await client.query<any>(`
        SELECT COUNT(Id) count
        FROM Subscription__c
        WHERE Status__c = 'Active'
